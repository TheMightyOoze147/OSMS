...
ka = 54 - 0.8 * (hBS/1000 - delta_h); %hBS <= delta_h
...
L2 = -16.9 - 10 * log10(w) + 10 * log10(f_MHz) + 20 * log10(delta_h - hms/1000) - 10 + 0.354 * fi;
if L1 + L2 > 0
    LNLOS = L0 + L1 + L2;
else
    LNLOS = L0;
...
figure(1);
plot(d, FSPM_PL, d, UMiNLOS_PL, d, COST231_PL, d, LLOS, d, LNLOS);
...
xlabel('Расстояние [m]')
legend('FSPM', 'UMiNLOS', 'COST231', 'Walfish-Ikegami LLOS', 'Walfish-Ikegami LNLOS');
